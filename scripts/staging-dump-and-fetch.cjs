...
function main() {
  const STAGING_SSH_HOST = getEnv('STAGING_SSH_HOST') || getEnv('SERVER_HOST');
  const STAGING_SSH_USER = getEnv('STAGING_SSH_USER', 'root');
  const STAGING_SSH_PORT = getEnv('STAGING_SSH_PORT', '22');
  const STAGING_SSH_KEY_PATH = getEnv('STAGING_SSH_KEY_PATH', '');
  ...
  const remoteScript = [
    ...
    `echo \"🔎 Validando container postgres-staging...\"`,
    "docker ps --format '{{.Names}}' | grep -q '^postgres-staging$' || { echo '❌ Container postgres-staging não está em execução'; exit 1; }",
    ...
    "if [ -z \"$DB_NAME\" ]; then DB_NAME=$(docker exec postgres-staging bash -lc 'echo -n \"${POSTGRES_DB:-projetogran}\"'); fi",
    'echo "🔎 DB_USER=$DB_USER DB_NAME=$DB_NAME"',
    'echo "🔎 Validando conexão..."',
    "docker exec postgres-staging env PGPASSWORD=\"$DB_PASS\" psql -h localhost -U \"$DB_USER\" -d \"$DB_NAME\" -tAc \"SELECT current_database();\" | grep -q ^$DB_NAME$ || { echo \"❌ Conexão ao DB $DB_NAME falhou\"; exit 1; }",
    'echo "🔎 Realizando pg_dump (custom format)..."',
    `docker exec postgres-staging env PGPASSWORD=\"$DB_PASS\" pg_dump -h localhost -U \"$DB_USER\" -d \"$DB_NAME\" -Fc > \"${remoteDumpPath}\"`,
