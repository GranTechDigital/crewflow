...
CMD ["sh", "-lc", "mkdir -p /app/data /app/relatorios; FILE=/app/data/vinculos.xlsx; if [ ! -f \"$FILE\" ]; then CAND=$(ls -1t /app/data/*.xlsx 2>/dev/null | head -n1); if [ -n \"$CAND\" ]; then FILE=\"$CAND\"; fi; fi; if [ -f \"$FILE\" ] && [ -f /app/scripts/vincular-funcionarios-contratos.js ]; then echo \"🔗 Executando vinculos com: $FILE\"; node scripts/vincular-funcionarios-contratos.js \"$FILE\" || true; if [ -f /app/relatorio-vinculos-funcionarios.xlsx ]; then cp -f /app/relatorio-vinculos-funcionarios.xlsx /app/relatorios/relatorio-vinculos-funcionarios.xlsx || true; fi; fi; npm start"]
