...
import { ChartBarIcon, UserGroupIcon, AcademicCapIcon, HeartIcon, BuildingOfficeIcon, TruckIcon, ClockIcon, ExclamationTriangleIcon, CheckCircleIcon, TrophyIcon, StarIcon, ArrowTrendingUpIcon } from "@heroicons/react/24/outline";
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, LabelList } from "recharts";
import * as XLSX from "xlsx";

type KPISet = {
  periodoDias: number;
  porSetor: { setor: string; qtdTarefas: number; duracaoMediaAtuacaoMs: number; tempoMedioConclusaoMs: number; reprovações: number }[];
  porRemanejamento: {
...
export default function RelatorioSLA() {
  ...
  const [activeTab, setActiveTab] = useState<'simples' | 'detalhado'>('detalhado');
  ...
  const exportDetalhadoXLSX = useCallback(() => {
    ...
    porRemanejamentoValidos.forEach((r) => {
      setoresDisponiveis.forEach((s) => {
        const per = (r.periodosPorSetor || []).find((x) => x.setor.toUpperCase() === s.toUpperCase());
        const durEntry = (r.duracaoPorSetorMs || []).find((x) => x.setor.toUpperCase() === s.toUpperCase());
        const mediaEntry = (r.temposMediosPorSetor || []).find((x) => x.setor.toUpperCase() === s.toUpperCase());
        rows.push({
          Remanejamento: String(r.remanejamentoId),
          Funcionario: r.funcionario?.nome || "",
          Matricula: r.funcionario?.matricula || "",
          Setor: s,
          Inicio: per?.inicio ? new Date(per.inicio).toLocaleString('pt-BR') : "",
          Fim: per?.fim ? new Date(per.fim).toLocaleString('pt-BR') : "",
          Duracao: durEntry?.ms ? fmtMs(durEntry.ms) : "",
          MediaConclusao: mediaEntry?.tempoMedioMs ? fmtMs(mediaEntry.tempoMedioMs) : "",
  ...
  const detalhadoKPIs = useMemo<DetalhadoKPIs>(() => {
    ...
    const mediaTotalMs = total ? Math.round(porRemanejamentoValidos.reduce((a, r) => a + (r.totalDurMs || 0), 0) / total) : 0;
    ...
    porRemanejamentoValidos.forEach((r) => {
      ...
      if (!hasLog && (r.periodosPorSetor || []).some((p) => (p.setor || '').toUpperCase() === 'LOGISTICA')) logisticCount++;
    ...
    Object.entries(durBySetor).forEach(([setor, ms]) => {
      if (!setorMaisLento || ms > setorMaisLento.ms) setorMaisLento = { setor, ms };
    });
    const reprovsTotal = porSetorBase.reduce((acc, s) => acc + (s.reprovações || 0), 0);
    const setorMaiorConclusao = porSetorBase.reduce<{ setor: string; ms: number } | null>((acc, s) => {
      const ms = s.tempoMedioConclusaoMs || 0;
      if (!acc || ms > acc.ms) return { setor: s.setor, ms };
      return acc;
    }, null);
    const percentLogistica = total ? Math.round((logisticCount / total) * 100) : 0;
    return { total, mediaTotalMs, setorMaisLento, reprovsTotal, setorMaiorConclusao, percentLogistica };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="w-full max-w-none mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Relatório de atendimentos realizados</h1>
        ...
        {data && (
          <div>
              ...
              <div className="border-b border-gray-200">
                <nav className="flex space-x-8 px-6" aria-label="Tabs">
                  {[
                    ...
                    { id: 'simples', name: 'Visão Simples', icon: ChartBarIcon },
                  ].map((tab) => (
                    <button
                      key={tab.id}
            ...
            {activeTab === 'simples' && (
            <Transition appear show={!!data} enter="transform transition duration-300" enterFrom="opacity-0 translate-y-2" enterTo="opacity-100 translate-y-0">
              <div className="rounded-2xl bg-white shadow-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <ChartBarIcon className="h-5 w-5 text-indigo-600" />
                    <h3 className="text-lg font-semibold text-gray-800">Tempo médio de conclusão por setor (h)</h3>
                  </div>
                  <div className="flex items-center gap-3 text-sm">
                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-blue-50 text-blue-700">
                      <ClockIcon className="h-4 w-4" /> Média geral: {mediaGeralHoras}h
                    </span>
                  </div>
                </div>
                <div className="h-48">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="horas" fill="url(#barGrad)" radius={[8,8,0,0]}>
              ...
              <div className="mt-6 rounded-2xl bg-white shadow-xl overflow-x-auto">
                <div className="px-4 py-3 flex items-center justify-between bg-gray-50 rounded-t-2xl">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-gray-800">Tempo médio por setor — por remanejamento</h3>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-500">{porRemanejamentoValidos.length} linha(s)</span>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setPage((p) => Math.max(1, p - 1))}
                        disabled={page <= 1}
                        className={`px-2 py-1 text-xs border rounded ${page <= 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100'}`}
                      >Anterior</button>
                      <span className="text-xs text-gray-600">Página {page} de {totalPages}</span>
                      <button
                        onClick={() => setPage((p) => Math.min(totalPages, p + 1))}
                        disabled={page >= totalPages}
                        className={`px-2 py-1 text-xs border rounded ${page >= totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100'}`}
                      >Próxima</button>
                    </div>
                  </div>
                </div>
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-100 sticky top-0 z-10">
                    <tr>
                      ...
                      <th className="text-left px-3 py-2"><span className="inline-flex items-center gap-1"><ClockIcon className="h-4 w-4 text-blue-600" /> Total</span></th>
                      {setoresDisponiveis.map((s) => {
                        ...
                        return (
                          <th key={`head-${s}`} className="text-left px-3 py-2"><span className="inline-flex items-center gap-1"><Icon className="h-4 w-4 text-gray-600" /> {s}</span></th>
                        );
                      })}
                    </tr>
                  </thead>
                  <tbody>
                    {pageData.map((r) => (
                      <tr key={`det-${r.remanejamentoId}`} className="border-t odd:bg-gray-50 hover:bg-gray-50">
                        <td className="px-3 py-2 text-gray-800">{String(r.remanejamentoId)}</td>
                        <td className="px-3 py-2 text-gray-800">{r.funcionario?.nome} ({r.funcionario?.matricula})</td>
                        <td className="px-3 py-2 text-gray-800"><span className="inline-flex items-center px-2 py-1 rounded-full bg-blue-50 text-blue-700">{fmtMs(r.totalDurMs)}</span></td>
                        {setoresDisponiveis.map((s) => {
                          const entry = (r.temposMediosPorSetor || []).find((x) => x.setor.toUpperCase() === s.toUpperCase());
                          const ms = entry ? entry.tempoMedioMs : 0;
                          const show = ms >= MIN_VALID_MS;
                          return (
                            <td key={`cell-${r.remanejamentoId}-${s}`} className="px-3 py-2 text-gray-800">
                              {show ? (
                                <span className="inline-flex items-center px-2 py-1 rounded-full bg-green-50 text-green-700">{fmtMs(ms)}</span>
                              ) : (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch">
                <div className="flex flex-col gap-4 lg:col-span-1">
                  <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-4 flex items-center justify-between">
                    <div>
                    <p className="text-sm font-medium text-gray-600">Total de Remanejamentos Concluídos</p>
                    <p className="text-2xl font-bold text-gray-900">{data?.porRemanejamento?.length ?? 0}</p>
                    </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-4 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-600">Média duração total</p>
                      <p className="text-2xl font-bold text-gray-900">{fmtMs(detalhadoKPIs.mediaTotalMs)}</p>
                  ...
                  <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <p className="text-sm font-medium text-gray-600">Ranking por setor (rápido → lento)</p>
                    ...
                    <div className="space-y-2">
                      {rankingSetores.map((s, idx) => {
                        ...
                        return (
                          <div key={`rank-${s.setor}`} className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              {idx === 0 ? (
                  ...
                  <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-4 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-600">Média de conclusão (geral)</p>
                      <p className="text-2xl font-bold text-gray-900">{mediaGeralHoras}h</p>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {setoresDisponiveis.map((s) => {
                          ...
                          return (
                            <span key={`badge-${s}`} className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${cls}`}>{s}: {h}h</span>
                          );
                        })}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      {topReprovacoes.length > 0 ? (
                        topReprovacoes.map(([tipo, qtd]) => (
                          <div key={`rep-${tipo}`} className="flex items-center justify-between">
                    ...
                    <div className="h-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData}>
                          ...
                          <Tooltip />
                          <Legend />
                          <Bar dataKey="horas" fill="url(#barGrad2)" radius={[8,8,0,0]}>
                            <LabelList dataKey="horas" position="top" />
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                        ...
                        <thead className="bg-gray-50 sticky top-0 z-10">
                          <tr>
                            ...
                            {setoresDisponiveis.map((s) => {
                              ...
                              const Icon = iconMap[s] || ChartBarIcon;
                              return (
                                <th key={`head-det-${s}`} className="text-left px-3 py-2 text-xs font-medium text-gray-600 uppercase tracking-wider whitespace-nowrap"><span className="inline-flex items-center gap-1"><Icon className="h-4 w-4 text-gray-600" /> {s}</span></th>
                              );
                            })}
                            <th className="text-left px-3 py-2 text-xs font-medium text-gray-600 uppercase tracking-wider whitespace-nowrap"><span className="inline-flex items-center gap-1"><ClockIcon className="h-4 w-4 text-gray-600" /> Total</span></th>
                          </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                          {pageData.map((r) => (
                            <tr key={`det-d-${r.remanejamentoId}`} className="hover:bg-gray-50">
                              ...
                              {setoresDisponiveis.map((s) => {
                                ...
                                const durEntry = (r.duracaoPorSetorMs || []).find((x) => x.setor.toUpperCase() === s.toUpperCase());
                                return (
                                  <td key={`cell-d-${r.remanejamentoId}-${s}`} className="px-3 py-2 text-gray-800 whitespace-nowrap">
                                    {periodEntry ? (
                                      <div className="flex flex-col gap-1">
                                        <span className="inline-flex items-center px-2 py-1 rounded-full bg-indigo-50 text-indigo-700">Início: {new Date(periodEntry.inicio).toLocaleString('pt-BR')}</span>
                                        <span className="inline-flex items-center px-2 py-1 rounded-full bg-purple-50 text-purple-700">Fim: {new Date(periodEntry.fim).toLocaleString('pt-BR')}</span>
                                        <span className="inline-flex items-center px-2 py-1 rounded-full bg-green-50 text-green-700">Duração: {durEntry?.ms ? fmtMs(durEntry.ms) : (() => { const ms = Math.max(0, new Date(periodEntry.fim).getTime() - new Date(periodEntry.inicio).getTime()); return ms ? fmtMs(ms) : '—'; })()}</span>
                                      </div>
                                    ) : (
                                      <span className="inline-flex items-center px-2 py-1 rounded-full bg-gray-100 text-gray-500">—</span>
                                    )}
                              ...
                              <td className="px-3 py-2 text-gray-800 whitespace-nowrap"><span className="inline-flex items-center px-2 py-1 rounded-full bg-blue-50 text-blue-700">{fmtMs(r.totalDurMs)}</span></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
