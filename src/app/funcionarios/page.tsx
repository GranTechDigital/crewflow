...
export default function Home() {
  ...
  const fetchDados = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/dados?t=${Date.now()}`, {
        cache: "no-store",
      });
  ...
  const handleSincronizar = async () => {
    ...
    if (result.success) {
      ...
      setSyncError(
        result.error || "Erro na sincronização após todas as tentativas"
      );
    }
  ...
  const getFilteredData = () => {
    ...
    const filteredData = dados.filter((pessoa) => {
      ...
      const matchesStatus =
        !filters.status ||
        (pessoa.status &&
          pessoa.status.toLowerCase() === filters.status.toLowerCase());
      const matchesDepartamento =
        !filters.departamento || pessoa.departamento === filters.departamento;
      const matchesCentroCusto =
        !filters.centroCusto || pessoa.centroCusto === filters.centroCusto;
  ...
  const dadosPagina = filteredData.slice(
          ...
          <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-2">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[11px] text-gray-500">
                Total:{" "}
                {(dashboard?.funcionariosPorStatusFolha || []).reduce(
            ...
            <div className="flex flex-wrap gap-1.5">
              {[...(dashboard?.funcionariosPorStatusFolha || [])]
                .sort((a, b) => b.count - a.count)
                .map((item) => {
                  const statusLabel = item?.status || "Sem status";
                  const ringClass =
                    getStatusConfig(statusLabel).ring || "ring-gray-600/20";
                  return (
                    <div
                      key={statusLabel}
                      className={`inline-flex items-center gap-1 px-2 py-1 text-[11px] rounded-md ${getStatusColor(
                        statusLabel,
                        "badge"
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
              ...
              <div>
                ...
                >
                  ...
                  {Array.from(
                    new Set(dados.map((p) => p.status).filter(Boolean))
                  ).map((status) => (
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFilters((prev) => ({ ...prev, funcao: e.target.value }))
                  }
                  className="w-full bg-white border border-gray-200 rounded-md px-2 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200"
                >
                  <option value="">Todas</option>
                  {Array.from(
                    new Set(dados.map((p) => p.funcao).filter(Boolean))
                  ).map((funcao) => (
              ...
              <div>
                ...
                >
                  <option value="">Todos</option>
                  {Array.from(
                    new Set(dados.map((p) => p.departamento).filter(Boolean))
                  ).map((depto) => (
              ...
              <div>
                ...
                >
                  <option value="">Todos</option>
                  {Array.from(
                    new Set(dados.map((p) => p.centroCusto).filter(Boolean))
                  ).map((cc) => (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {dadosPagina.map((pessoa) => (
                  <tr
                    ...
                    className={`hover:bg-gray-50 cursor-pointer transition-colors duration-150 ${getStatusBorder(
                      pessoa.status || undefined
                    )}`}
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5 whitespace-nowrap text-xs">
                      <span
                        className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[11px] font-medium ${getStatusColor(
                          pessoa.status || undefined,
                          "default"
                        )}`}
                      >
            ...
            <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(
                      selectedPessoa.status || undefined,
                      "modal"
                    )}`}
                  >
              ...
              <div className="flex-1 overflow-y-auto p-4">
                <div className="grid grid-cols-2 gap-4">
                  {Object.entries(
                    isEditing ? editedPessoa! : selectedPessoa
                  ).map(([key, value]) => {
                    if (
                      key === "id" ||
                      key === "dataCriacao" ||
                      ...
                      <div key={key} className="bg-gray-50 rounded-md p-2.5">
                        ...
                        {isEditing ? (
                          ...
                          <p className="text-sm text-gray-900">
                            {key === "dataNascimento"
                              ? value
                                ? formatarDataBR(value as string)
                                : "-"
                              : value || "-"}
                          </p>
                        )}
                  ...
                  <div className="space-y-2">
                    {selectedPessoa.dataCriacao && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-gray-900">
                          {formatarDataBR(selectedPessoa.dataCriacao)}
                        </span>
                      </div>
                    )}
                    {selectedPessoa.dataAtualizacao && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-gray-900">
                          {formatarDataBR(selectedPessoa.dataAtualizacao)}
                        </span>
                      </div>
                    )}
                    {selectedPessoa.dataExclusao && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-red-600">
                          {formatarDataBR(selectedPessoa.dataExclusao)}
                        </span>
                      </div>
                    )}
                  </div>
