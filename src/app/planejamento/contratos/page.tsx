...
export default function ContratosPage() {
  ...
  const filteredContratos = contratos.filter(contrato => {
    ...
    const matchesCliente = !clienteFilter || contrato.cliente.toLowerCase().includes(clienteFilter.toLowerCase());
  ...
  const paginatedContratos = filteredContratos.slice(startIndex, startIndex + itemsPerPage);

  // Obter lista única de clientes para filtro
  const uniqueClientes = Array.from(new Set(contratos.map(c => c.cliente))).sort();
  ...
  function openAddModal() {
    ...
    setForm({ numero: '', nome: '', cliente: '', dataInicio: '', dataFim: '', status: 'Ativo' });
  ...
  function openEditModal(contrato: Contrato) {
    ...
    const statusOption = STATUS_OPTIONS.find(s => s.value === contrato.status) || STATUS_OPTIONS[0];
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!formData.numero || !formData.nome || !formData.cliente || !formData.dataInicio || !formData.dataFim || !formData.status) {
        ...
        <div className="bg-white shadow rounded-lg p-6 mb-6">
                        ...
                        <Menu.Item key={cliente}>
                          {({ active }) => (
                            <button
                              ...
                              className={`${active ? 'bg-gray-100' : ''} block w-full text-left px-4 py-2 text-sm text-gray-700 truncate`}
                            >
                              {cliente}
                            </button>
                          )}
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex items-center justify-between text-sm text-gray-600">
              <span>
                Mostrando {paginatedContratos.length} de {filteredContratos.length} contratos
                {filteredContratos.length !== contratos.length && ` (${contratos.length} total)`}
              </span>
              {(searchTerm || statusFilter || clienteFilter) && (
                <button
                  onClick={() => {
        ...
        {loading ? (
          ...
          <>
                  ...
                  {paginatedContratos.map((contrato) => (
                    <tr key={contrato.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-gray-900">{contrato.nome}</div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {formatDateWithoutTimezone(contrato.dataInicio)} até {formatDateWithoutTimezone(contrato.dataFim)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="lg:hidden space-y-4">
              {paginatedContratos.map((contrato) => (
                <div key={contrato.id} className="bg-white shadow rounded-lg p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        <h3 className="text-sm font-medium text-gray-900">{contrato.nome}</h3>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(contrato.status)}`}>
                          {contrato.status}
                        </span>
                      </div>
                      <p className="mt-1 text-sm text-gray-600">#{contrato.numero}</p>
                      <p className="mt-1 text-sm text-gray-600">{contrato.cliente}</p>
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(currentPage * itemsPerPage, filteredContratos.length)}</span> de{' '}
                  <span className="font-medium">{filteredContratos.length}</span> resultados
                </p>
              </div>
              <div>
                ...
                <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900 mb-6">
                    <div className="flex items-center justify-between">
                      <span>{editingContrato ? 'Editar Contrato' : 'Novo Contrato'}</span>
                    </div>
                  </Dialog.Title>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                          ...
                          <div className="relative">
                            <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                              <span className="flex items-center">
                                <span className={`inline-block w-2 h-2 rounded-full mr-2 ${selectedStatus.color.split(' ')[0]}`}></span>
                    ...
                    <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                      ...
                      >
                        {editingContrato ? 'Atualizar Contrato' : 'Criar Contrato'}
                      </button>
                    </div>
                  </form>
                </Dialog.Panel>
