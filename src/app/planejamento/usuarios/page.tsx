...
export default function UsuariosPlanejamentoPage() {
  return (
    <ProtectedRoute
    ...
    >
      <UsuariosEquipeContent sectorLabel="Planejamento" />
    </ProtectedRoute>
  );
}
...
function UsuariosEquipeContent({ sectorLabel }: { sectorLabel: string }) {
  ...
  const isGestorPlanejamento =
    usuario?.equipe?.includes("Planejamento") &&
    usuario?.equipe?.includes("(Gestor)");
  // Cadastro de novos membros: exclusivamente Gestor do setor
  const canCreateUser = isGestorPlanejamento;
  const canManageTeam = isGestorPlanejamento || isAdmin;
