  ...
  try {
    ...
    for (const rf of remanejamentos) {
      ...
      const deriveSetor = (t: any) => {
        const evNome = (t.eventosStatus || [])
          .map((e: any) => e.equipe?.nome)
          .filter(Boolean)[0] as string | undefined;
        const evSet = toSetor(evNome);
        if (evSet === "RH" || evSet === "MEDICINA" || evSet === "TREINAMENTO")
          return evSet;
        ...
        for (const setor of setores) {
          ...
          if (!existente) {
            intervalosPorSetor[setor] = {
              inicio: inicioSetor.toISOString(),
              fim: fimSetor.toISOString(),
            };
          } else {
            const atualInicio = new Date(existente.inicio);
            const atualFim = new Date(existente.fim);
            const novoInicio =
              inicioSetor < atualInicio ? inicioSetor : atualInicio;
