...
export async function PUT(
  ...
  try {
    ...
    if (!status && !observacoes && !dataConclusao && !dataLimite) {
      return NextResponse.json(
    ...
    if (dataLimite !== undefined) {
      const dataLimiteAnterior = tarefaAtual.dataLimite ? tarefaAtual.dataLimite.toISOString() : null;
      const dataLimiteNova = dataLimite ? new Date(dataLimite).toISOString() : null;
        ...
        try {
          await prisma.historicoRemanejamento.create({
            data: {
              ...
              valorAnterior: dataLimiteAnterior ? new Date(dataLimiteAnterior).toLocaleDateString('pt-BR') : 'Não definida',
              valorNovo: dataLimiteNova ? new Date(dataLimiteNova).toLocaleDateString('pt-BR') : 'Removida',
              usuarioResponsavel: 'Sistema', // Pode ser melhorado para capturar o usuário real
              observacoes: observacoes || undefined
            }
          });
        } catch (historicoError) {
          console.error('Erro ao registrar histórico da data limite:', historicoError);
          // Não falha a atualização se o histórico falhar
        }
      }
    }
    ...
    if (status !== undefined) {
      await atualizarStatusTarefasFuncionario(tarefaAtual.remanejamentoFuncionarioId);
    }
...
async function atualizarStatusTarefasFuncionario(remanejamentoFuncionarioId: string) {
  try {
    ...
    if (!remanejamentoFuncionario) {
      console.error('RemanejamentoFuncionario não encontrado:', remanejamentoFuncionarioId);
      return;
    }
    ...
    if (todasConcluidas && remanejamentoFuncionario.statusPrestserv === 'APROVADO') {
      await verificarConclusaoSolicitacao(remanejamentoFuncionario.solicitacaoId);
    }
  } catch (error) {
