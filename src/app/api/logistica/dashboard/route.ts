...
export async function GET(request: NextRequest) {
  try {
    ...
    const funcionariosPendentes = funcionariosPorStatusTarefas.find(
      item => item.statusTarefas === 'PENDENTE'
    )?._count.id || 0;

    const funcionariosAptos = funcionariosPorStatusTarefas.find(
      item => item.statusTarefas === 'CONCLUIDO'
    )?._count.id || 0;

    const funcionariosSubmetidos = funcionariosPorStatusPrestserv.find(
      item => item.statusPrestserv === 'SUBMETIDO'
    )?._count.id || 0;

    const funcionariosAprovados = funcionariosPorStatusPrestserv.find(
      item => item.statusPrestserv === 'APROVADO'
    )?._count.id || 0;

    const funcionariosRejeitados = funcionariosPorStatusPrestserv.find(
    ...
    const funcionariosAtencao = await prisma.remanejamentoFuncionario.findMany({
      where: {
        OR: [
          // Funcionários com tarefas concluídas mas ainda não submetidos
          {
