...
export async function GET(request: NextRequest) {
  try {
    ...
    if (funcionarioId)
      funcionariosConditions.funcionarioId = parseInt(funcionarioId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validação de funcionários inaptos (admissão prox.mês ou demitido)
    ...
    try {
      ...
      if (inaptos.length > 0) {
        return NextResponse.json(
          {
            error:
              'Um ou mais funcionários estão inaptos para remanejamento/alocação por status de folha (demitido/rescisão)',
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { id, status, analisadoPor, observacoes } = body;
