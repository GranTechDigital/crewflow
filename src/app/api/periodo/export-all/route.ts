...
export async function GET(request: NextRequest) {
  try {
    ...
    const dadosExcel = dados.map(item => ({
      ...
      'Data Upload': item.upload?.dataUpload ? new Date(item.upload.dataUpload).toLocaleDateString('pt-BR') : '',
      'Usuário Upload': item.upload?.usuario || '',
      'Data Criação': new Date(item.createdAt).toLocaleDateString('pt-BR'),
      'Data Atualização': new Date(item.updatedAt).toLocaleDateString('pt-BR')
    }));
