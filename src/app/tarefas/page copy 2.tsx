...
export default function TarefasPage() {
  ...
  // Removido estado tarefas artificial - trabalhando diretamente com solicitacoes
  ...
  const [tarefaSelecionada, setTarefaSelecionada] = useState<TarefaRemanejamento | null>(
    null
  );
  ...
  // Estados para visão por funcionários
  const [visaoPorFuncionarios, setVisaoPorFuncionarios] = useState(false);
  const [tarefasSelecionadas, setTarefasSelecionadas] = useState<Set<string>>(
    new Set()
  );
  const [funcionariosExpandidos, setFuncionariosExpandidos] = useState<
    Set<string>
  >(new Set());
  const [aprovandoTarefas, setAprovandoTarefas] = useState(false);
  
  // Estados para modal de confirmação de aprovação de tarefas
  const [mostrarModalConfirmacaoAprovacao, setMostrarModalConfirmacaoAprovacao] = useState(false);
  const [chaveGrupoSelecionado, setChaveGrupoSelecionado] = useState<string>("");
  const [tarefasParaAprovar, setTarefasParaAprovar] = useState<TarefaRemanejamento[]>([]);
  const [funcionarioSelecionado, setFuncionarioSelecionado] = useState<{nome: string, matricula: string} | null>(null);
  ...
  const getRemanejamentosFiltrados = () => {
    ...
    solicitacoes.forEach((solicitacao: SolicitacaoRemanejamento) => {
      solicitacao.funcionarios?.forEach((remanejamento: RemanejamentoFuncionario) => {
        ...
        const tarefasFiltradas = remanejamento.tarefas?.filter((tarefa: TarefaRemanejamento) => {
          // Lógica especial para status concluído
          let matchStatus = !filtroStatus;
          if (filtroStatus === "CONCLUIDO") {
            matchStatus = tarefa.status === "CONCLUIDO" || tarefa.status === "CONCLUIDA";
  ...
  const getRemanejamentosPaginados = () => {
    ...
    const remanejamentosOrdenados = [...remanejamentosFiltrados].sort((a, b) => {
  ...
  // Funções para gerenciar seleção de tarefas
  const toggleSelecionarTarefa = (tarefaId: string, event?: React.MouseEvent | React.ChangeEvent | null) => {
    // Prevenir comportamento padrão e propagação para evitar que a página role para o topo
    if (event) {
      event.preventDefault();
      event.stopPropagation();
      
      // Garantir que o foco permaneça no elemento
      const target = event.target as HTMLElement;
      if (target) {
        // Usar setTimeout para garantir que o foco seja mantido após o evento
        setTimeout(() => {
          target.focus();
        }, 0);
      }
  ...
  const selecionarTodasTarefasRemanejamento = (remanejamentoId: string) => {
    const remanejamentosFiltrados = getRemanejamentosFiltrados();
    const remanejamento = remanejamentosFiltrados.find(r => String(r.id) === remanejamentoId);
    
    if (!remanejamento?.tarefas) return;

    const tarefasPendentes = remanejamento.tarefas.filter(
      (tarefa) => tarefa.status !== "CONCLUIDA" && tarefa.status !== "CONCLUIDO"
    );

    const novaSelecao = new Set(tarefasSelecionadas);
    const todasSelecionadas = tarefasPendentes.every((t) =>
      novaSelecao.has(t.id)
    );

    if (todasSelecionadas) {
      // Desselecionar todas
      tarefasPendentes.forEach((t) => novaSelecao.delete(t.id));
    } else {
      // Selecionar todas
      tarefasPendentes.forEach((t) => novaSelecao.add(t.id));
    }
  ...
  // Função para abrir o modal de confirmação de aprovação
  const abrirModalConfirmacaoAprovacao = (chaveGrupo: string) => {
    const [funcionarioId, remanejamentoId] = chaveGrupo.split("_");
    
    // Verificar se há tarefas selecionadas
    if (tarefasSelecionadas.size === 0) {
      toast.error("Nenhuma tarefa selecionada");
      return;
    }
    
    // Buscar todas as tarefas dos remanejamentos
    const todasTarefas: TarefaRemanejamento[] = [];
    const remanejamentosFiltrados = getRemanejamentosFiltrados();
    ...
    const tarefasFuncionario = Array.from(tarefasSelecionadas)
      .map(tarefaId => todasTarefas.find(t => t.id === tarefaId))
      .filter((tarefa): tarefa is TarefaRemanejamento => {
        if (!tarefa) return false;
        const funcionario = tarefa.remanejamentoFuncionario?.funcionario;
        const funcionarioIdTarefa = String(funcionario?.id);
        const remanejamentoIdTarefa = String(tarefa.remanejamentoFuncionario?.remanejamentoId);
        return (
          funcionarioIdTarefa === funcionarioId &&
          remanejamentoIdTarefa === remanejamentoId
        );
      });

    if (tarefasFuncionario.length === 0) {
      toast.error("Nenhuma tarefa selecionada para este funcionário");
      return;
    }

    // Obter informações do funcionário para exibir no modal
    const primeiroItem = tarefasFuncionario[0];
    const funcionario = primeiroItem.remanejamentoFuncionario?.funcionario;
    ...
    setTarefasParaAprovar(tarefasFuncionario);
    setChaveGrupoSelecionado(chaveGrupo);
    setMostrarModalConfirmacaoAprovacao(true);
  };

  // Função para fechar o modal de confirmação
  const fecharModalConfirmacaoAprovacao = () => {
    setMostrarModalConfirmacaoAprovacao(false);
    setChaveGrupoSelecionado("");
    setTarefasParaAprovar([]);
    setFuncionarioSelecionado(null);
  };

  const aprovarTarefasSelecionadas = async (chaveGrupo: string) => {
    const [funcionarioId, remanejamentoId] = chaveGrupo.split("_");
    ...
    const tarefasFuncionario = Array.from(tarefasSelecionadas).filter(
      (tarefaId) => {
        const tarefa = todasTarefas.find((t) => t.id === tarefaId);
        if (!tarefa) return false;
        const funcionario = tarefa.remanejamentoFuncionario?.funcionario;
        const funcionarioIdTarefa = String(funcionario?.id);
        const remanejamentoIdTarefa = String(tarefa.remanejamentoFuncionarioId);
        return (
          funcionarioIdTarefa === funcionarioId &&
          remanejamentoIdTarefa === remanejamentoId
        );
    ...
    try {
      setAprovandoTarefas(true);

      const promessas = tarefasFuncionario.map(async (tarefaId) => {
        ...
        if (!response.ok) {
          throw new Error(`Erro ao aprovar tarefa ${tarefaId}`);
        }
      ...
      // Remover tarefas aprovadas da seleção
      const novaSelecao = new Set(tarefasSelecionadas);
      tarefasFuncionario.forEach((id) => novaSelecao.delete(id));
      setTarefasSelecionadas(novaSelecao);
      ...
      toast.success(
        `${tarefasFuncionario.length} tarefa(s) aprovada(s) com sucesso!`
      );
      
      // Fechar o modal de confirmação
      fecharModalConfirmacaoAprovacao();
    } catch (error) {
      console.error("Erro ao aprovar tarefas:", error);
      toast.error("Erro ao aprovar algumas tarefas");
    } finally {
      setAprovandoTarefas(false);
    }
  };
  ...
  const getRemanejamentosParaVisaoFuncionarios = () => {
    ...
    return remanejamentosFiltrados.map(remanejamento => {
      ...
      tarefasFiltradas.sort((a: TarefaRemanejamento, b: TarefaRemanejamento) => {
        const statusA = a.status === "CONCLUIDA" || a.status === "CONCLUIDO" ? 1 : 0;
        const statusB = b.status === "CONCLUIDA" || b.status === "CONCLUIDO" ? 1 : 0;
  ...
  // Componente para visão por funcionários
  const VisaoPorFuncionarios = () => {
    const remanejamentosComTarefas = getRemanejamentosParaVisaoFuncionarios();
    // console.log('Remanejamentos com tarefas:', remanejamentosComTarefas);
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {remanejamentosPaginados.map(
                  ({ funcionario, tarefas, remanejamento }) => {
                            ...
                            <div className="flex items-center gap-2">
                              <div>
                                ...
                                <div className="">
                                  ...
                                  {
                                    tarefas[0].remanejamentoFuncionario
                              ...
                              <div className="bg-gray-50 border-t border-gray-200">
                                <div className="px-6 py-4">
                                  <div className="flex justify-between items-center mb-3">
                                    ...
                                    <button
                                      className="inline-flex items-center px-2 py-1 text-xs font-medium text-green-600 bg-white border border-green-300 rounded hover:bg-green-50 transition-colors"
                                      onClick={() => {
                                        // Abrir modal de confirmação para aprovar tarefas selecionadas
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                      <thead className="bg-gray-100">
                                        <tr>
                                          <th
                                            scope="col"
                                            className="text-left text-xs font-medium text-gray-600 uppercase tracking-wider"
                                          ></th>
                                          <th
                                            scope="col"
                                                  ...
                                                  {podeSelecionar ? (
                                                    <div
                                                      onClick={(e) => {
                                                        e.preventDefault();
                                                        e.stopPropagation();
                                                        toggleSelecionarTarefa(
                                                          tarefa.id,
                                                          e
                                                        );
                                                        return false;
                                                      }}
                                                      className="h-4 w-4 flex items-center justify-center border border-gray-300 rounded cursor-pointer"
                                                      style={{
                                                        backgroundColor:
                                                          tarefasSelecionadas.has(
                                                            tarefa.id
                                                          )
                                                            ? "#2563eb"
                                                            : "white",
                                                    ...
                                                    <div className="h-4 w-4 flex items-center justify-center">
                                                      <CheckCircleIcon className="h-4 w-4 text-green-600" title="Tarefa concluída" />
                                                    </div>
                                                  )}
                                                </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-r from-blue-500 to-blue-600 p-6 rounded-lg shadow-lg text-white">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-blue-100 text-xs">Total de Tarefas</p>
                <p className="text-2xl font-bold">{tarefasFiltradas.length}</p>
              </div>
              <ClipboardDocumentCheckIcon className="h-8 w-8 text-blue-200" />
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-yellow-500 to-yellow-600 p-6 rounded-lg shadow-lg text-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{statsStatus.pendentes}</p>
                {atrasadas > 0 && (
          ...
          <div className="bg-gradient-to-r from-green-500 to-green-600 p-6 rounded-lg shadow-lg text-white">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-green-100 text-xs">Concluídas</p>
                <p className="text-2xl font-bold">{statsStatus.concluidas}</p>
              </div>
          ...
          <div className="bg-gradient-to-r from-purple-500 to-purple-600 p-6 rounded-lg shadow-lg text-white">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-purple-100 text-xs">Em Andamento</p>
                <p className="text-2xl font-bold">{statsStatus.emAndamento}</p>
              </div>
  ...
  const ListaTarefas = () => {
    if (visaoPorFuncionarios) {
      return <VisaoPorFuncionarios />;
    }

    const remanejamentosPaginados = getRemanejamentosPaginados();
    const remanejamentosFiltrados = getRemanejamentosFiltrados();

    if (loading) {
      return <div className="text-center py-10">Carregando remanejamentos...</div>;
    }

    if (error) {
      return (
        <div className="text-center py-10 text-red-500">
          Erro ao carregar remanejamentos: {error}
        </div>
      );
    }

    if (remanejamentosFiltrados.length === 0) {
      return (
        <div className="text-center py-10">Nenhum remanejamento encontrado.</div>
      );
    }
    ...
    return (
      <div className="bg-white border-slate-400 border-1 rounded-lg shadow-lg overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200" id="tb1">
            <thead className="bg-slate-100">
              <tr>
                <th
                  scope="col"
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {remanejamentosPaginados.map((remanejamento) => {
                const funcionario = remanejamento.funcionario!;
                const solicitacao = (remanejamento as any).solicitacao as SolicitacaoRemanejamento;
                const chaveRemanejamento = `${funcionario.id}_${remanejamento.id}`;
                const expandido = funcionariosExpandidos.has(chaveRemanejamento);
                
                // Calcular progresso das tarefas
                const totalTarefas = remanejamento.tarefas?.length || 0;
                const tarefasConcluidas = remanejamento.tarefas?.filter(
                ...
                return (
                  <React.Fragment key={remanejamento.id}>
                    {/* Linha principal do remanejamento */}
                    <tr
                      className={`hover:bg-gray-50 group ${
                        funcionario.status === "INATIVO" || funcionario.status === "DEMITIDO"
                          ? "border-l-4 border-l-red-500 bg-red-50/30"
                          : ""
                      }`}
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center space-x-3">
                          <button
                            onClick={() => toggleExpandirFuncionario(chaveRemanejamento)}
                            className="text-gray-500 hover:text-gray-700"
                          >
                            <ChevronRightIcon
                              className={`h-4 w-4 transform transition-transform ${
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {precisaAtencao && tipoAlerta && (
                              <div className="group relative ml-2">
                                ...
                                />
                                <div
                                  className={`absolute z-10 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity duration-200 bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 text-xs rounded-lg border shadow-lg max-w-xs ${tipoAlerta.classes}`}
                                >
                                  <div className="font-medium mb-1">
                                    ⚠️ Atenção Necessária
                                  </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-600">
                        <div className="flex items-center gap-2">
                          <div>
                            <div className="text-xs font-medium text-gray-900">
                              {solicitacao.contratoOrigem?.nome || 'N/A'} → {solicitacao.contratoDestino?.nome || 'N/A'}
                            </div>
                            <div className="text-xs text-gray-500">
                              Sol. #{solicitacao.numeroSolicitacao}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-600">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-medium">{totalTarefas}</span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-xs font-medium sticky right-0 bg-white shadow-md z-10">
                        {(() => {
                          const tarefasPendentesRemanejamento = remanejamento.tarefas?.filter(
                            (t) => tarefasSelecionadas.has(t.id) && t.status !== "CONCLUIDA" && t.status !== "CONCLUIDO"
                          ) || [];
                          
                          if (tarefasPendentesRemanejamento.length === 0) {
                            return (
                          ...
                          return (
                            <button
                              className="inline-flex items-center px-3 py-1 text-xs font-medium text-white bg-green-600 border border-transparent rounded hover:bg-green-700 transition-colors"
                              onClick={() => abrirModalConfirmacaoAprovacao(chaveRemanejamento)}
                              disabled={aprovandoTarefas}
                            >
                              <ClipboardDocumentCheckIcon className="w-3 h-3 mr-1" />
                              Aprovar Selecionadas ({tarefasPendentesRemanejamento.length})
                            </button>
                          );
                        })()}
                      </td>
                    ...
                    {/* Sublinhas de tarefas expandíveis */}
                    {expandido && remanejamento.tarefas && remanejamento.tarefas.map((tarefa: TarefaRemanejamento) => {
                      // Determinar classes de status
                      let statusClasses = "px-2 py-1 text-xs rounded-full";
                      if (tarefa.status === "PENDENTE")
                        statusClasses += " bg-yellow-100 text-yellow-800";
                      else if (tarefa.status === "CONCLUIDA" || tarefa.status === "CONCLUIDO")
                      ...
                      const atrasada = dataLimite && dataLimite < hoje && 
                        tarefa.status !== "CONCLUIDA" && tarefa.status !== "CONCLUIDO";

                      return (
                        <tr key={`${remanejamento.id}-${tarefa.id}`} className="bg-gray-50 hover:bg-gray-100">
                          <td className="px-6 py-3 whitespace-nowrap">
                            <div className="flex items-center space-x-3">
                              <div className="w-4"></div> {/* Espaço para alinhamento */}
                              <div className="flex items-center space-x-2">
                                {tarefa.status !== "CONCLUIDA" && tarefa.status !== "CONCLUIDO" ? (
                                  <input
                                    type="checkbox"
                                    checked={tarefasSelecionadas.has(tarefa.id)}
                                    onChange={() => toggleSelecionarTarefa(tarefa.id)}
                          ...
                          <td className="px-6 py-3 whitespace-nowrap">
                            <span className={statusClasses}>
                              {tarefa.status === "PENDENTE" ? "Pendente" :
                               tarefa.status === "CONCLUIDA" || tarefa.status === "CONCLUIDO" ? "Concluída" :
                               tarefa.status}
                            </span>
                          </td>
                          <td className="px-6 py-3 whitespace-nowrap">
                          ...
                          <td className="px-6 py-3 whitespace-nowrap text-right text-xs font-medium sticky right-0 bg-gray-50 shadow-md z-10">
                            <button
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-xs text-gray-700">
                Mostrando{" "}
                <span className="font-medium">{remanejamentosPaginados.length}</span>{" "}
                de{" "}
                <span className="font-medium">{remanejamentosFiltrados.length}</span>{" "}
                remanejamentos
              </p>
            </div>
            <div>
              ...
              >
                ...
                {Array.from({
                  length: Math.ceil(remanejamentosFiltrados.length / itensPorPagina),
                }).map((_, index) => (
                  <button
                    key={index}
                    onClick={() => setPaginaAtual(index + 1)}
                    className={`relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-xs font-medium ${
                      paginaAtual === index + 1
                ...
                <button
                  onClick={() =>
                    setPaginaAtual(
                      paginaAtual <
                        Math.ceil(remanejamentosFiltrados.length / itensPorPagina)
                        ? paginaAtual + 1
                        : paginaAtual
                    )
                  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          <button
            onClick={() => setVisaoPorFuncionarios(!visaoPorFuncionarios)}
            className={`flex items-center px-4 py-2 border rounded-md shadow-sm text-xs font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 ${
              visaoPorFuncionarios
                ? "border-blue-500 bg-blue-500 text-white hover:bg-blue-600 focus:ring-blue-500"
                : "border-slate-500 text-slate-500 hover:bg-slate-200 focus:ring-slate-500"
            }`}
          >
            <UserGroupIcon className="h-5 w-5 mr-2" />
            {visaoPorFuncionarios ? "Visão Lista" : "Visão por Funcionários"}
          </button>
          <button
      ...
      {activeTab === "cards" && <ListaTarefas />}
      {activeTab === "funcionarios" && <VisaoPorFuncionarios />}
      ...
      {/* Modal de Confirmação de Aprovação de Tarefas */}
      {mostrarModalConfirmacaoAprovacao && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <h3 className="text-lg font-semibold text-gray-900">
                    Confirmar Aprovação de Tarefas
                  </h3>
              ...
              <button
                onClick={fecharModalConfirmacaoAprovacao}
              ...
              <div className="mb-6">
                {funcionarioSelecionado ? (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    <h4 className="text-sm font-medium text-blue-900 mb-2">Informações do Funcionário</h4>
                    <p className="text-sm text-blue-800">
                      <strong>Nome:</strong> {funcionarioSelecionado.nome}
                    </p>
                    <p className="text-sm text-blue-800">
                      <strong>Matrícula:</strong> {funcionarioSelecionado.matricula}
                    </p>
                  </div>
                ) : (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                    <h4 className="text-sm font-medium text-yellow-900 mb-2">Carregando informações...</h4>
                    <p className="text-sm text-yellow-800">
                      Aguarde enquanto carregamos os dados do funcionário.
                    </p>
                  </div>
                )}
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <h4 className="text-sm font-medium text-green-900 mb-3">
                    Tarefas Selecionadas ({tarefasParaAprovar.length})
                  </h4>
                  
                  {tarefasParaAprovar.length > 0 ? (
                    <div className="max-h-48 overflow-y-auto">
                      <div className="space-y-2">
                        {tarefasParaAprovar.map((tarefa, index) => (
                          <div key={tarefa.id} className="flex items-start p-3 bg-white rounded border border-green-200">
            ...
            <div className="flex justify-end space-x-3 p-6 border-t border-gray-200 bg-gray-50">
              ...
              <button
                onClick={() => aprovarTarefasSelecionadas(chaveGrupoSelecionado)}
                className="px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                disabled={aprovandoTarefas || tarefasParaAprovar.length === 0}
              >
                {aprovandoTarefas ? (
                  <div className="flex items-center">
                  ...
                  `Aprovar ${tarefasParaAprovar.length} Tarefa${tarefasParaAprovar.length !== 1 ? 's' : ''}`
                )}
              </button>
            </div>
          </div>
