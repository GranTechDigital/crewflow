...
export default function MedicinaPage() {
  ...
  const fetchTarefas = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/logistica/tarefas?responsavel=MEDICINA');
  ...
  const exportarParaExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Tarefas Medicina');

      // Gerar nome do arquivo com data atual
      const dataAtual = new Date().toLocaleDateString('pt-BR').replace(/\//g, '-');
      const nomeArquivo = `tarefas-medicina-${dataAtual}.xlsx`;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Tarefas - Medicina do Trabalho</h1>
        <p className="text-gray-600">Gerencie as tarefas e pendências do setor de Medicina do Trabalho</p>
      </div>
