...
export default function UsuariosMedicinaPage() {
  return (
    <ProtectedRoute
      requiredPermissions={ROUTE_PROTECTION.MEDICINA.requiredPermissions}
      requiredEquipe={ROUTE_PROTECTION.MEDICINA.requiredEquipe}
    >
      <UsuariosEquipeContent sectorLabel="Medicina" />
    </ProtectedRoute>
  );
}
...
function UsuariosEquipeContent({ sectorLabel }: { sectorLabel: string }) {
  ...
  const canCreateUser = isGestorMedicina;
  const canManageTeam = isGestorMedicina || isAdmin;
