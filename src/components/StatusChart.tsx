...
const StatusChart: React.FC<StatusChartProps> = ({
  ...
  const activeStatuses = Object.entries(statusCount).filter(
    ([_, count]) => count > 0
  );

  if (activeStatuses.length === 0) {
    return (
      <div className="text-xs text-gray-400 italic">Nenhum funcionário</div>
    );
  }
  ...
  return (
    <div className="w-full">
      ...
      <div
        className={`w-full ${heightClasses[height]} bg-gray-200 rounded-full overflow-hidden flex`}
      >
        {activeStatuses.map(([status, count], index) => {
          ...
          return (
            <div
              ...
              title={`${status}: ${count} funcionário${
                count !== 1 ? "s" : ""
