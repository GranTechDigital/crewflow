...
export default function TarefasPadraoModal({
  isOpen,
  onClose,
  funcionario,
  onSuccess,
}: TarefasPadraoModalProps) {
  const { showToast } = useToast();
  const [setoresSelecionados, setSetoresSelecionados] = useState<string[]>([]);
  const [tarefasPadrao, setTarefasPadrao] = useState<TarefasPadraoData | null>(
    null
  );
  ...
  const criarTarefasPadrao = async () => {
    ...
    if (funcionario.statusPrestserv === "SUBMETIDO") {
      showToast(
        "Não é possível criar novas tarefas quando o prestserv está submetido",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Erro ao reprovar tarefas padrões");
      }
            ...
            <div>
              ...
              {funcionario && (
                <p className="text-sm text-gray-600 mt-1">
                  Funcionário: {funcionario.nome} (Matrícula:{" "}
            ...
            <>
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {tarefasPadrao.tarefasPadrao[setor]?.length || 0}{" "}
                            tarefas
                          </p>
                        </div>
                      </label>
                    ...
                    {setoresSelecionados.map((setor) => (
                      ...
                      >
                        ...
                        <div className="space-y-2">
                          {tarefasPadrao?.tarefasPadrao[setor]?.map(
                            (tarefa, index) => (
                              <div
                                key={index}
              ...
              <div className="flex justify-end space-x-3">
                ...
                >
                  {criandoTarefas
                    ? "Criando..."
                    : `REPROVAR TAREFAS (${setoresSelecionados.reduce(
                        (total, setor) =>
                          total +
                          (tarefasPadrao?.tarefasPadrao[setor]?.length || 0),
                        0
