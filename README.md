...
### 📝 Notas de Remanejamento — Filtros de Folha (Nov/2025)
- Oculta funcionários com status de folha indicando `demitido` ou `admissão próximo mês` nas telas de criação de remanejamentos.
- Frontend: regras aplicadas na página `src/app/prestserv/remanejamentos/novo/page.tsx` e no componente `src/components/RemanejamentoModal.tsx`.
- Backend: reforço da regra na API `src/app/api/logistica/funcionarios/route.ts` considerando `tipo` (`alocacao`, `realocacao`, `desligamento`) e `contratoId` condicional.
- Benefício: evita seleção e submissão de solicitações com funcionários inaptos.
- Sugestão futura: centralizar util `isInaptoFolha(statusFolha: string)` em `src/utils` e reutilizar em todas as listas.
